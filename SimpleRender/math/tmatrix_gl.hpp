...
namespace math {
	template<typename T>
	static inline auto lookatMatrix(const TVectorN<T, 3>& position,
	...
	template<typename T>
	static inline auto viewportMatrix(const T& width, const T& height) {
	...
	template<typename T>
	static inline auto viewportMatrixReflectY(const T& width, const T& height) {
		auto scaleVec = TVectorN<T, 3>(width / T(2), height / T(2), T(1));
		auto scaleMatrix = scale3T(scaleVec);
		auto translateMatrix = TMatrix3<T>(T(1));
		translateMatrix[2][0] = (width - T(1)) / T(2);
	...
	// attention: should pass into the float parameter
	template<typename T>
	static inline auto frontviewMatrix(const TVectorN<T, 3>& pointThis, const TVectorN<T, 3>& pointThat) {
		T l = pointThis[0];
		T b = pointThis[1];
		T n = pointThis[2];
		T r = pointThat[0];
	...
	template<typename T>
	static inline auto perspectiveMatrix(const TVectorN<T, 3>& pointThis, const TVectorN<T, 3>& pointThat) {
		T n = pointThis[2];
		T f = pointThat[2];
	...
	**/
	template<typename T>
	static inline auto perspectiveMatrix(const T& fovy, const T& aspect, const T& n, const T& f) {
