...
namespace math {
	...
	static inline auto transpose(const TMatrixN<T, w, h>& matrixN) {
	...
	// -------------------- not universal --------------------
	template<typename T>
	static inline auto asMat3(const TMatrix4<T> mat4) {
	...
	template<typename T>
	static inline auto asMat4(const TMatrix3<T> mat3) {
	...
	template<typename T>
	static inline auto translate(const TVectorN<T, 3> vec3) {
		TMatrix4<T> mat4(T(1));
		mat4[3][0] = vec3[0];
		mat4[3][1] = vec3[1];
		mat4[3][2] = vec3[2];
	...
	template<typename T>
	static inline auto scale3T(const TVectorN<T, 3> vec3) {
		TMatrix3<T> mat3(T(1));
		mat3[0][0] = vec3[0];
		mat3[1][1] = vec3[1];
		mat3[2][2] = vec3[2];
	...
	template<typename T>
	static inline auto scale4T(const TVectorN<T, 3> vec3) {
