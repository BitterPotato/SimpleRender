...
WinForm::WinForm(int width, int height, FrameBuffer* framebuffer) {
...
void WinForm::runRender() const{
    while (screen_exit == 0 && screen_keys[VK_ESCAPE] == 0) {
        screen_dispatch();

        // function pointers
        if(WinForm::mKeyCallback)
            WinForm::mKeyCallback(screen_keys);
...
int WinForm::screen_close(void) const{
    if (screen_dc) {
        if (screen_ob) {
...
void WinForm::screen_dispatch(void) const {
    MSG msg;
    while (1) {
...
void WinForm::screen_update(void) const {
